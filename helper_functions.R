...
# takes in main disorder data frame (produced below), a list of icd9 and icd10 codes for the relevant cancer subtype, and the name of the subtype (e.g., 'breast')
...
cancer_subtype <- function(df, icd9, icd10, subtype){
  ...
  subtype_codes <- as.data.frame(t(apply(subtype_codes, 1, move_nas_to_right)), stringsAsFactors = FALSE)
...
combine_scales <- function(version = '', j_max = 20,  years = c(2015, 2015)){
  ...
  if (version == 'within_achb' | version == 'within_all'){
    ...
    for (i in seq(1: nrow(scales_achb))){
      scale_names[length(scale_names) + 1] <- paste0('score_', scales_achb$scale[i], '.csv')
    }
  } else if (version == 'across_achb' | version == 'across_all'){
    for (i in seq(0, j_max-1)){
      scale_names[length(scale_names) + 1] <- paste0('pseudo_scale_', as.character(i), '.csv')
    }
  }
  ...
  data_period <- filter(data_period, year_present != 0 & year >= years[1] & year <= years[length(years)]) # remove irrelevant years
  data_period$year_present <- NULL
  # by merging on data_period, we will include some previously excluded id's; this is desired, as those id's were previously removed when pseudo_scales() was run
  # (the id's in question were id's that were prescribed only drugs with invalid administration routes on given years)
  # some id's are also going to get removed from scales; this are the id's that do not have more than one year of continuous EHR 
  ...
  # replace NAs with 0s ("data_period" introduced NAs because it contains participants-years that had been removed from the precursors to "scales" when invalid administration routes were removed)
  ...
  scale_siz <- read.csv(paste0('output_files/pseudo_scale_size_', version_abr, '.csv'), header = TRUE)
  colnames(scale_siz) <- c('n')
  scale_siz$scale_name <- colnames(select(scales, -ends_with('_alt') & starts_with('score')))
  
  # export
  write.csv(scale_siz, paste0('output_files/pseudo_scale_size_', version_abr, '.csv'), row.names = FALSE)
...
### This function takes the participant-ID data frame and transforms it into a participant data frame; the medication burden is averaged.

## The function below takes as inputs: 
#   - `how`: the type of simulation/sampling ('across_all', 'across_achb', 'within_all', 'within_achb')
#   - `file_name`: name of the file with the outcomes to be read; it should have two columns, where the first one indicates
#      the presence (1) or absence (0) of the outcome, and the second column indicates the earliest date of diagnosis
#   - 'outcome_name': 'death', 'dementia', or 'delirium'
#   - `year_range`: range (inclusive) for which the mean of the burden score will be calculated

## It outputs:
#   - a data frame where each ID is an individual observation and the mean prescribing burden for that individual is given;
#     it also includes various demographic and other variables that will be used as covariates
...
prepare_scales <- function(version, file_name, outcome_name, year_range = c(2000, 2005)){
  ...
  outcome[[outcome_name]][outcome[[date_outcome]] %in% c("1900-01-01", "1901-01-01", "1902-02-02", "1903-03-03", "2037-07-07")] <- NA
  outcome[[date_outcome]][outcome[[date_outcome]] %in% c("1900-01-01", "1901-01-01", "1902-02-02", "1903-03-03", "2037-07-07")] <- NA
  ...
  # sex, age
  sex_age <- read.csv('age_sex_formatted.csv', sep = '|') %>% select(id, sex, birth_year, birth_date)
  # year first present in prescription sample
  data_period <- read.csv('data_period_long.csv')
  colnames(data_period)[colnames(data_period) == 'eid'] <- 'id'
  year_first <- filter(data_period, year_present !=0) %>% group_by(id) %>% summarise(year_first = min(year))
  ...
  # for those for whom sampling started in 2015 or later choose that year as first year of achb sampling
  df$year_achb_first[df$year_first >= start_year] <- df$year_first[df$year_first >= start_year]
  ...
  # subsample with only participants for whom there is data before sampling period (i.e., remove participants that were first recorded after 2015)
  df <- filter(df, year_first <= stop_year, )
  # calculate the number of sampling years, so we know how much to average over; also calculate the last year of AChB sampling
  sampling_time <- filter(data_period, year_present !=0 & year >= start_year & year <= stop_year) %>% group_by(id) %>% summarise(sampling_time = n())
  year_achb_last <- filter(data_period, year_present !=0 & year >= start_year & year <= stop_year) %>% group_by(id) %>% summarise(year_achb_last = max(year))
  ...
  df$age <- as.numeric(difftime(df$date_achb_last, as.Date(df$birth_date, format = '%Y-%m-%d'), units = 'days'))/365.25
  ...
  data_provider <- data_provider %>% group_by(id, year) %>% summarise(data_provider = Mode(data_provider))
  ...
  colnames(achb)[3:ncol(achb)] <- paste('score_', colnames(achb)[3:ncol(achb)], sep='')
  colnames(achb_alt)[3:ncol(achb_alt)] <- paste('score_', colnames(achb_alt)[3:ncol(achb_alt)], sep='')
  ...
  # data providers for each participant; for participants for which there is no data provider info for the given years, impute data provider from other years
  data_provider_sum <- meds %>% select(id, data_provider) %>% group_by(id) %>% summarise(data_provider = Mode(data_provider, na.rm = TRUE))
  dp_missing <- filter(data_provider_sum, is.na(data_provider)) %>% select(id)
  dp_missing <- merge(dp_missing, data_provider, by = 'id', all.x = TRUE)
  dp_missing$start_distance <- abs(dp_missing$year - start_year) # calculate absolute distance from start and finish of relevant prescribing period
  dp_missing$stop_distance <- abs(dp_missing$year - stop_year)
  dp_missing <- dp_missing %>% transform(min_distance = pmin(start_distance, stop_distance, na.rm = TRUE)) %>% arrange(min_distance)
  ...
  data_provider_sum <- merge(data_provider_sum, subset(dp_missing, select = c(id, data_provider_imputed)), by = 'id', all = TRUE)
  data_provider_sum$data_provider_imputed[is.na(data_provider_sum$data_provider_imputed)] <- data_provider_sum$data_provider[is.na(data_provider_sum$data_provider_imputed)]
  ...
  rm(data_period, data_provider, data_provider_sum, dp_missing, outcome, sex_age, year_achb_last, year_first)
  # this has imputed data providers for 30,638 participants; 1,941 participants remain without data providers and were assigned dp 0
  ...
  meds <- meds[, lapply(.SD, sum, na.rm = TRUE), by = id] # in cases of multiple years: sum across the years
  meds <- data.frame(meds)
  meds <- merge(df[, c('id', 'sex', 'age', 'data_provider', 'data_provider_imputed', 'sampling_time', outcome_name)], meds, by = 'id')
  meds <- meds %>% mutate(across(starts_with('score'), ~ . / sampling_time)) # # in cases of multiple years: average across the years
  ...
  if (outcome_name %in% c('dementia')) {
    meds <- merge(meds, cov_dis %>% select(-dementia, -dementia_date, -death, -death_date), 
                  by = 'id', all.x = TRUE)
  } else if (outcome_name %in% c('death')) {
    meds <- merge(meds, cov_dis %>% select(-death, -death_date), by = 'id', all.x = TRUE)
  } else{
  ...
  # if the date of occurrence is NA, change disease coding to 0, effectively removing cases after end of sampling
  disease_cols <- str_remove_all(names(meds)[str_detect(names(meds), "_date$")], "_date")
  disease_cols <- disease_cols[disease_cols != "birth"]
  meds <- meds %>% mutate(across(all_of(disease_cols), ~if_else(is.na(meds[[paste0(cur_column(), "_date")]]), 0, .)))
  
  # change some variables to factors
  meds <- meds %>% mutate(across(c(id, sex, mood_dis, diabetes, hypertension, hyperlip, psych_dis, hear_loss_any, cns_vasc,
                                   cns_infl, cns_atroph, cns_mov, cns_demyel, cns_parox, cns_other, cns_tbi, cns_cancer,
                                   vision_problem, sleep_dis_any, endocrine_dis, nutr_dis, metabolic_dis, cerebrovascular,
                                   respiratory, hepatic, flu, heart, dementia, cancer_colon, cancer_prostate, cancer_lung,
                                   cancer_breast, cancer_ovary,
                                   starts_with(c('data_prov', 'education', 'alc_freq', 'smoking', 'phys_act',
                                                 'depressed', 'lonely', 'soc_isol'))), as.factor))
